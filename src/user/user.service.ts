...
export class UserService {
   ...
   *
   * @param {Model<User>} userModel - The Mongoose model for the User schema,
   ...
   * @returns {Promise<Partial<User>>} - A promise that resolves to the newly created user data, excluding the password.
   * @throws {ConflictException} - If a user with the same username or email already exists.
   */
  public async register(registerUserDto: RegisterUserDto): Promise<Partial<User>> {
   ...
   * @returns {Promise<UserDocument>} - A promise that resolves to the user document if found.
   */
   ...
   * @returns {Promise<UserDocument>} - A promise that resolves to the user document if found.
   */
  async findByUsernameOrEmail(identifier: string): Promise<UserDocument> {
   ...
   * @returns {Promise<UserDocument>} - A promise that resolves to the user document if validation is successful.
   * @throws {NotFoundException} - If the user is not found.
   ...
   * @returns {Promise<Partial<User>>} - A promise that resolves to the updated user data, excluding the password.
   * @throws {NotFoundException} - If the user is not found.
   */
  public async updateUserName(userId: mongoose.Types.ObjectId, updateUserDto: UpdateUserDto): Promise<Partial<User>> {
